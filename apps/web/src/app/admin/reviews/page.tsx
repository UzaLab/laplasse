...
import { Loader2, Star, CheckCircle2, EyeOff, Trash2 } from 'lucide-react'
...
export default function AdminReviewsPage() {
  ...
  return (
    <AdminShell pageTitle="Avis">
      <div className="mb-6">
        <h2 className="text-xl font-extrabold text-slate-900">Avis &amp; Modération</h2>
              ...
              <div key={r.id} className="bg-white border border-slate-100 rounded-2xl p-5">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Content */}
                    {r.title && <p className="font-bold text-slate-900 mb-1">{r.title}</p>}
                    {r.content && <p className="text-sm text-slate-600 leading-relaxed">{r.content}</p>}
                  </div>

...
import { Loader2, Radio, RefreshCw, Truck } from 'lucide-react'
...
function matchesTab(job: PartnerDeliveryJob, tab: string) {
...
export default function LogisticsDispatchPage() {
  ...
  const { data: jobs = [], isLoading, isFetching, dataUpdatedAt } = useQuery({
    queryKey: ['logistics-dispatch-jobs'],
  ...
  const { data: fleet = [] } = useQuery({
    queryKey: ['logistics-dispatch-fleet'],
    queryFn: fetchPartnerFleet,
    enabled: ready && verified,
    refetchInterval: 30_000,
  })

  const activeFleet = useMemo(
    () => fleet.filter(c => c.status === 'ACTIVE'),
    [fleet],
  )
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['logistics-dispatch-jobs'] })
    },
      ...
      <div className="w-full min-w-0 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Assignez les courses entrantes à votre flotte en temps réel.</p>
          </div>
            ...
            {visibleJobs.map(job => (
              <li key={job.id}>
                <LogisticsDispatchJobCard
                  ...
                  selectedCourierId={selectedCourier[job.id] ?? ''}
                  onSelectCourier={courierId => setSelectedCourier(s => ({ ...s, [job.id]: courierId }))}
                  onAssign={() => {
                    const courierId = selectedCourier[job.id]
                    if (!courierId) {

    ...
    <LogisticsShell>
      <div className="w-full min-w-0 space-y-6">
        ...
        <form onSubmit={handleLink} className="bg-white rounded-2xl border border-slate-100 p-5 space-y-3">
          ...
          <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="Email compte livreur" className="w-full border border-slate-200 rounded-full px-4 py-2.5 text-sm" />
          <button type="submit" disabled={linking} className="w-full py-2.5 bg-slate-900 text-white rounded-full font-bold text-sm disabled:opacity-50">

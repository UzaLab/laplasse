  ...
  return (
    <ProfileShell>
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-[28px] p-8 text-white mb-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-56 h-56 bg-white/10 rounded-full blur-[50px] -translate-y-1/3 translate-x-1/3 pointer-events-none" />
        <div className="relative z-10">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="bg-white/20 rounded-2xl p-3 text-center">
              ...
              <p className="text-xs opacity-80 mt-0.5">Amis parrainés</p>
            </div>
            <div className="bg-white/20 rounded-2xl p-3 text-center">
              <p className="text-3xl font-black">{isLoading ? '…' : data?.total_points_earned ?? 0}</p>
              <p className="text-xs opacity-80 mt-0.5">Points gagnés</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Code */}
      <div className="bg-white rounded-[28px] border border-slate-100 p-6 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={copyCode}
            className="w-14 h-14 rounded-2xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center shrink-0 transition-colors"
          >
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-[#25D366] text-white font-bold text-sm hover:opacity-90 transition-opacity"
          >
            <span>💬</span> WhatsApp
          </button>
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({ title: 'LaPlasse', text: shareText, url: shareUrl })
              } else {
      ...
      <div className="bg-white rounded-[28px] border border-slate-100 overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="font-extrabold text-slate-900">Comment ça marche ?</h3>
        </div>
        ...
        <div className="bg-white rounded-[28px] border border-slate-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            <h3 className="font-extrabold text-slate-900 flex items-center gap-2">
              <Users size={16} className="text-slate-500" /> Vos filleuls
            </h3>
          </div>
          <div className="divide-y divide-slate-50">
              ...
              <div key={r.id} className="flex items-center gap-4 px-6 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">
                    {new Date(r.invited_user.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}
                  </p>
                </div>

  ...
  return (
    <ProfileShell>
      ...
      <div className={`bg-gradient-to-br ${cfg.color} rounded-[28px] p-8 text-white mb-6 relative overflow-hidden`}>
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-[60px] -translate-y-1/3 translate-x-1/3 pointer-events-none" />
        <div className="relative z-10">
          ...
          <div className="text-4xl font-black mb-4">{cfg.label}</div>
          <div className="flex items-end gap-3">
            ...
            {pointsToNext !== null && (
              <div className="pb-1 opacity-75 text-sm font-semibold flex items-center gap-1">
                <ChevronUp size={14} /> {pointsToNext} pts jusqu'au niveau suivant
              </div>
            )}
          </div>
          {nextTier && (
      ...
      <div className="bg-white rounded-[28px] border border-slate-100 overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="font-extrabold text-slate-900 flex items-center gap-2">
            <Trophy size={16} className="text-amber-500" /> Niveaux
          </h3>
        </div>
        <div className="divide-y divide-slate-50">
          {Object.entries(TIER_CONFIG).map(([key, t]) => {
            ...
            return (
              <div key={key} className={`flex items-center gap-4 px-6 py-4 ${isActive ? 'bg-slate-50' : ''}`}>
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${t.color} flex items-center justify-center text-lg shrink-0 ${isActive ? `ring-2 ${t.ring}` : ''}`}>
                  {t.emoji}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-extrabold ${isActive ? 'text-slate-900' : isReached ? 'text-slate-700' : 'text-slate-400'}`}>{t.label}</p>
                  <p className="text-xs text-slate-400">{tierData?.min ?? 0} points minimum</p>
                </div>
                {isActive && (
                  <span className="text-[10px] font-black bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full">Actuel</span>
                )}
                {!isActive && isReached && (
                  <span className="text-[10px] font-bold text-emerald-600">✓ Atteint</span>
                )}
              </div>
            )
          })}
      ...
      <div className="bg-white rounded-[28px] border border-slate-100 overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="font-extrabold text-slate-900 flex items-center gap-2">
            <Zap size={16} className="text-amber-500" /> Comment gagner des points
          </h3>
        </div>
        <div className="divide-y divide-slate-50">
          {[
